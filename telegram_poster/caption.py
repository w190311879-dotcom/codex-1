...
def build_caption(title, keywords, fixed_content, url=None, max_length=MAX_TELEGRAM_CAPTION_LENGTH):
    title_text = truncate_text(title or "Untitled", 180)
    keywords_text = keyword_line(keywords)

    required_parts = [part for part in [title_text, keywords_text] if part]
    required = "\n\n".join(required_parts)
    ...
    parts = [title_text, keywords_text, fixed_text]
